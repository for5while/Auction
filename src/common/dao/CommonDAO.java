...
public ArrayList<ShopRankBean> getShopRank() {
	...
	public Map<String, Map<String, Integer>> getChartdata(){
		...
		try {
			String sql = "SELECT * FROM keyword ORDER BY kw_search_count DESC LIMIT 0,7";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()) {
			...
			sql = "SELECT DATE_format(`mb_reg_datetime`,'%c월%d일') AS `date`, COUNT(*) AS `cnt` FROM member GROUP BY `date` ORDER BY `date` ASC";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()) {
			...
			for(int i=0;i<5;i++) {
				sql = "SELECT COUNT(it_successful_price) AS 'price_cnt' FROM item WHERE it_successful_price > ? AND it_successful_price <= ?";
