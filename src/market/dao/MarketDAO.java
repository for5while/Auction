...
public class MarketDAO {
	...
	public MarketBeanForCount getCounts(String mb_id) {
		...
		try {
			...
			sql = "SELECT mb_point FROM member WHERE mb_id=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, mb_id);
			rs = pstmt.executeQuery();
			if(rs.next()) {
	...
	public boolean PasswordChk(String mb_id, String mb_password) {
		...
		try {
			sql = "SELECT mb_no FROM member WHERE mb_id=? and mb_password=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, mb_id);
	...
	public int UpdateMemberInfo(MemberBean memberBean) {
		int updateResult = 0;
		try {
			sb = new StringBuffer("UPDATE  member SET ");
	...
	public int duplicationCheck(String mb_id, String nick){
		...
		String db_id = null;
		try {
			sql = "SELECT mb_id FROM member WHERE mb_nick=?";
			pstmt = con.prepareStatement(sql);
